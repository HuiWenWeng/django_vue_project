  ...
  <div>
    <form method="post" class="form">
      ...
      <p>
        ...
        <input type="text" name="name" v-model="course_name" maxlength="100" required="" id="id_name">
      </p>
      <p>
      ...
      <p>
        ...
        <span v-if="course_grade == 9">
          <Multiselect v-model="student_list" :options="student_list_source.filter(checkGrade9)" :multiple="true" :close-on-select="false" :clear-on-select="false" :preserve-search="true" placeholder="Choose the student(s)" label="name" track-by="name" :preselect-first="true" style="display:inline-block;width: 300px;padding-bottom:10px;padding-left:10px">
            <template slot="selection" slot-scope="{ values, search, isOpen }"><span class="multiselect__single" v-if="values.length" v-show="!isOpen">{{ values.length }} options selected</span></template>
          </Multiselect>
        </span>
        <span v-if="course_grade == 10">
          <Multiselect v-model="student_list" :options="student_list_source.filter(checkGrade10)" :multiple="true" :close-on-select="false" :clear-on-select="false" :preserve-search="true" placeholder="Choose the student(s)" label="name" track-by="name" :preselect-first="true" style="display:inline-block;width: 300px;padding-bottom:10px;padding-left:10px">
            <template slot="selection" slot-scope="{ values, search, isOpen }"><span class="multiselect__single" v-if="values.length" v-show="!isOpen">{{ values.length }} options selected</span></template>
          </Multiselect>
        </span>
        <span v-if="course_grade == 11">
          <Multiselect v-model="student_list" :options="student_list_source.filter(checkGrade11)" :multiple="true" :close-on-select="false" :clear-on-select="false" :preserve-search="true" placeholder="Choose the student(s)" label="name" track-by="name" :preselect-first="true" style="display:inline-block;width: 300px;padding-bottom:10px;padding-left:10px">
            <template slot="selection" slot-scope="{ values, search, isOpen }"><span class="multiselect__single" v-if="values.length" v-show="!isOpen">{{ values.length }} options selected</span></template>
          </Multiselect>
        </span>
        <span v-if="course_grade == 12">
          <Multiselect v-model="student_list" :options="student_list_source.filter(checkGrade12)" :multiple="true" :close-on-select="false" :clear-on-select="false" :preserve-search="true" placeholder="Choose the student(s)" label="name" track-by="name" :preselect-first="true" style="display:inline-block;width: 300px;padding-bottom:10px;padding-left:10px">
            <template slot="selection" slot-scope="{ values, search, isOpen }"><span class="multiselect__single" v-if="values.length" v-show="!isOpen">{{ values.length }} options selected</span></template>
          </Multiselect>
        </span>
      </p>
